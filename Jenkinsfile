...
node {
    ...
    stage('Execute') {
       ...
       if (isUnix()) {
                sh 'docker run --network="host" --rm pytest-with-src --executor "remote" --browser "chrome" .'
            }
        else {
                /* Make sure you have shared the folder and set full permissions for this folder "%WORKSPACE%\\allure-results"*/
                bat 'docker run --network="host" --rm pytest-with-src --executor "remote" --browser "chrome" .'
            }
        }
    }
